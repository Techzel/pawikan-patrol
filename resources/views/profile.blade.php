                ...
                <div class="glass-dark rounded-2xl p-6 border border-green-500/30">
                    <div class="text-center">
                        ...
                        <div class="flex justify-center gap-2 mb-3">
                            {!! auth()->user()->getVerificationBadge() !!}
                ...
                <div class="glass-dark rounded-2xl p-6 border border-green-500/30">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        ...
                        <div class="bg-gradient-to-br from-yellow-500/10 to-yellow-600/10 rounded-xl p-4 border border-yellow-500/20">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-3">
                                <div class="text-center">
                                    <div class="text-lg font-bold text-white" data-stat="quiz-games">{{ $quizGamesPlayed }}</div>
                                    <div class="text-xs text-gray-300">Played</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-lg font-bold text-yellow-400" data-stat="quiz-best-score">{{ $bestScore * 10 }}</div>
                                    <div class="text-xs text-gray-300">Points</div>
                                </div>
                            </div>
                            
                            <a href="{{ route('games.quiz') }}" class="block w-full bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 text-center py-2 rounded-lg text-xs font-medium transition-colors">
                                Play Now
                            </a>
                        </div>
                    </div>
                            ...
                            @foreach($recentSessions as $session)
                                <div class="flex items-center justify-between p-3 bg-deep-800/30 rounded-lg">
                                    ...
                                    <div class="text-right">
                                        <div class="text-green-400 font-bold text-sm">{{ $session->game_type === 'quiz' ? $session->score * 10 : $session->score }} pts</div>
                                        <div class="text-gray-300 text-xs">{{ sprintf('%02d:%01d.%02d', floor($session->time_spent / 60), $session->time_spent % 60, ($session->time_spent - floor($session->time_spent)) * 100) }}</div>
                                    </div>
                                </div>
                            @endforeach
                        @else
