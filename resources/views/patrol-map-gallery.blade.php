        ...
        <div class="flex flex-wrap justify-center gap-3">
            ...
            @foreach($availableTypes as $type)
                @php
                    ...
                    $icon = $typeIcons[$type] ?? '📌';
                @endphp
                <button class="filter-btn glass-dark px-6 py-3 rounded-full text-sm font-semibold cinzel-text hover:bg-ocean-600/20 transition-all" data-filter="{{ $type }}">
                    {{ $icon }} {{ $label }}
                </button>
            @endforeach
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        @forelse($reports as $index => $report)
            <div class="featured-report glass-dark rounded-2xl overflow-hidden gallery-item"
                 data-type="{{ strtolower($report['report_type']) }}">
                ...
                <div class="report-header">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-2 flex-wrap">
                            <span class="badge badge-{{ strtolower($report['report_type']) }} text-xs">
                                {{ $typeIcons[$report['report_type']] ?? '📌' }} {{ ucfirst($report['report_type']) }}
                            </span>
                            @if($index === 0)
                            ...
                            @endif
                            @if($report['priority'])
                                <span class="badge badge-{{ strtolower($report['priority']) }} text-xs">
                ...
                @if(!empty($report['images']))
                    <div class="p-6 bg-gradient-to-b from-ocean-900/20 to-transparent">
                        ...
                        @endphp
                        <div class="compact-image-grid {{ $gridClass }}">
                            @foreach($report['images'] as $imageIndex => $image)
                                <div class="compact-image relative cursor-pointer group" 
                                     onclick="openLightbox('{{ asset('storage/' . $image) }}', {{ json_encode($report) }})">
                            ...
                            <div class="report-content">
                                ...
                                <div class="report-section">
                                    ...
                                    <div class="info-row">
                                        ...
                                        <span class="info-value cinzel-text">{{ $report['location'] }}</span>
                                    </div>
                                    
                                    <div class="info-row">
                                        <span class="info-label cinzel-text">📅 Report Date</span>
                                        <span class="info-value cinzel-text">{{ $report['reported_at'] }}</span>
                                    </div>
                                    
                                    @if($report['incident_datetime'])
                                        <div class="info-row">
                                            <span class="info-label cinzel-text">🕐 Incident Time</span>
                                            <span class="info-value cinzel-text">{{ $report['incident_datetime'] }}</span>
                                        </div>
                                    @endif
                                    
                                    <div class="info-row">
                                        <span class="info-label cinzel-text">👤 Reported By</span>
                                        <span class="info-value cinzel-text">{{ $report['reported_by'] }}</span>
                                    </div>
                                    
                                    @if($report['weather_conditions'])
                                        <div class="info-row">
                                            <span class="info-label cinzel-text">🌤️ Weather</span>
                                            <span class="info-value cinzel-text">{{ $report['weather_conditions'] }}</span>
                                        </div>
                                    @endif
                                </div>
                                    ...
                                    <div class="report-section">
                                        ...
                                        @if($report['turtle_species'])
                                            <div class="info-row">
                                                ...
                                                <span class="info-value cinzel-text">{{ $report['turtle_species'] }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($report['turtle_gender'])
                                            <div class="info-row">
                                                ...
                                                <span class="info-value cinzel-text">{{ ucfirst($report['turtle_gender']) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($report['turtle_count'])
                                            <div class="info-row">
                                                ...
                                                <span class="info-value cinzel-text">{{ $report['turtle_count'] }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($report['egg_count'])
                                            <div class="info-row">
                                                ...
                                                <span class="info-value cinzel-text">{{ $report['egg_count'] }}</span>
                                            </div>
                                        @endif
                                            ...
                                            <div class="info-row">
                                                ...
                                                <span class="info-value cinzel-text">
                                                    <span class="px-2 py-1 rounded-full text-xs font-semibold
                                                        {{ $report['turtle_condition'] === 'healthy' ? 'bg-green-500/20 text-green-400' : '' }}
                                                        {{ $report['turtle_condition'] === 'injured' ? 'bg-red-500/20 text-red-400' : '' }}
                                                        {{ $report['turtle_condition'] === 'sick' ? 'bg-yellow-500/20 text-yellow-400' : '' }}">
                                ...
                                @if($report['latitude'] && $report['longitude'])
                                    <div class="report-section" style="grid-column: 1 / -1;">
                                        ...
                                        <div class="info-row">
                                            ...
                                            <span class="info-value cinzel-text font-mono">{{ $report['latitude'] }}</span>
                                        </div>
                                        <div class="info-row">
                                            <span class="info-label cinzel-text">Longitude</span>
                                            <span class="info-value cinzel-text font-mono">{{ $report['longitude'] }}</span>
                                        </div>
                                    </div>
                                @endif
                                ...
                                @if($report['additional_notes'])
                                    <div class="report-section notes-section">
                                        ...
                                        <p class="text-gray-300 text-sm cinzel-text leading-relaxed">{{ $report['additional_notes'] }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
...
<script>
    ...
    function openLightbox(imageSrc, report) {
        ...
        currentImages = report.images.map(img => '{{ asset("storage") }}/' + img);
