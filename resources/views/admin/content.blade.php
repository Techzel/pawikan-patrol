...
@section('title', 'Content Management - DENR Admin')
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-12 relative z-10">
        ...
        <header class="mb-8">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors border border-gray-700">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white/10 backdrop-blur-md rounded-xl border border-white/20 overflow-hidden h-fit">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.content.storytelling.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
                            ...
                            @foreach($files as $file)
                                <div class="bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex items-center justify-between group hover:border-blue-500/30 transition-colors">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <button onclick="openPdfPreview('{{ $file['url'] }}', {{ json_encode($file['title']) }}, {{ json_encode($file['description']) }}, '{{ date('M d, Y', strtotime($file['published_date'])) }}')" class="p-2 text-gray-400 hover:text-blue-400 bg-gray-700/50 hover:bg-gray-700 rounded-lg transition-colors" title="View Preview">
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50">
        <div class="bg-gray-800 border border-white/10 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl transform transition-all scale-100">
            ...
            <div class="flex justify-end gap-3">
                ...
                <form id="deleteForm" action="{{ route('admin.content.storytelling.delete') }}" method="POST">
                    @csrf
                    @method('DELETE')
