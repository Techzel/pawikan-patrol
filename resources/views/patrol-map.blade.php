...
@endphp
<div class="{{ $isPatrollerOrAdmin ? '' : 'pt-20' }}">
    <!-- Header -->
                ...
                <div class="p-5 flex items-center justify-between border-b border-white/5 bg-white/5 backdrop-blur-sm flex-shrink-0 relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 relative z-10">
                        ...
                        <div>
                            ...
                            <p class="text-emerald-400/80 text-xs font-medium uppercase tracking-wider">{{ $stats['last_updated'] }}</p>
                        </div>
                    </div>
                ...
                <div class="p-5 space-y-6 flex-1 overflow-y-auto min-h-0 custom-scrollbar">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="bg-white/5 hover:bg-emerald-500/10 border border-white/5 hover:border-emerald-500/40 rounded-2xl p-4 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.03] hover:shadow-[0_20px_40px_rgba(0,0,0,0.3),0_0_20px_rgba(16,185,129,0.2)] group relative overflow-hidden cursor-pointer">
                            ...
                            <div class="flex items-baseline gap-1">
                                <span class="text-3xl font-bold bg-gradient-to-r from-white to-slate-400 group-hover:from-white group-hover:to-emerald-400 bg-clip-text text-transparent counter-up transition-all duration-500" data-target="{{ $stat['value'] }}">0</span>
                    ...
                    <div class="bg-white/5 border border-white/5 rounded-2xl p-5 relative overflow-visible group/chart">
                        ...
                        <div class="relative h-32 w-full mb-2">
                            ...
                            <svg class="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 100">
                                ...
                                <!-- Area Fill -->
                                <path d="{{ $areaD }}" fill="url(#graphGradient)" stroke="none" vector-effect="non-scaling-stroke"/>
                                
                                <!-- Line Path -->
                                <path d="{{ $d }}" fill="none" stroke="#10b981" stroke-width="2" vector-effect="non-scaling-stroke" stroke-linecap="round" stroke-linejoin="round" class="drop-shadow-lg"/>
                            </svg>
                            
                            <!-- Interactive Points Overlay -->
                            <div class="absolute inset-0 w-full h-full">
                                @foreach($points as $i => $point)
                                    <div class="absolute group/point z-10" style="left: {{ $point['x'] }}%; top: {{ $point['y'] }}%; transform: translate(-50%, -50%);">
                                        <!-- Hover Target Area -->
                                        ...
                                        <div class="absolute bottom-full mb-3 left-1/2 -translate-x-1/2 bg-slate-800/95 backdrop-blur-md border border-white/10 rounded-lg py-2 px-3 shadow-xl opacity-0 translate-y-2 group-hover/point:opacity-100 group-hover/point:translate-y-0 transition-all duration-200 pointer-events-none min-w-[100px] z-50">
                                            <div class="flex flex-col items-center gap-0.5">
                                                <span class="text-emerald-400 text-[10px] font-bold uppercase tracking-wider">{{ $point['data']['month'] }}</span>
                                                <span class="text-white text-sm font-bold">{{ $point['data']['count'] }} Reports</span>
                                            </div>
                                            <!-- Arrow -->
                            ...
                            <div class="absolute top-full left-0 w-full flex justify-between text-[10px] text-slate-500 font-medium pt-2">
                                 @foreach($points as $point)
                                    <span style="position: absolute; left: {{ $point['x'] }}%; transform: translateX(-50%); white-space: nowrap;">
                                        {{ $point['data']['month'] }}
                                    </span>
                                 @endforeach
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Reporters Summary -->
                         ...
                         <div class="space-y-3">
                             ...
                             <div class="bg-white/5 border border-white/5 rounded-2xl overflow-hidden divide-y divide-white/5">
                                 @foreach($stats['top_reporters'] as $reporter => $count)
                                     @php
                                     ...
                                     @endphp
                                     <div class="p-3 {{ $isMe ? 'bg-emerald-500/10' : 'hover:bg-white/5' }} transition-colors relative group">
                                         <!-- Background Progress Bar -->
                                         <div class="absolute inset-y-0 left-0 bg-emerald-500/5 transition-all duration-1000 ease-out" style="width: {{ $percent }}%"></div>
                                         
                                         <div class="relative flex items-center justify-between z-10">
                                             <div class="flex items-center gap-3">
                                                 <div class="w-8 h-8 rounded-full {{ $isMe ? 'bg-emerald-500 text-white' : 'bg-slate-700 text-slate-300' }} flex items-center justify-center text-xs font-bold ring-2 {{ $isMe ? 'ring-emerald-500/30' : 'ring-white/5' }}">
                                                     {{ substr($reporter, 0, 1) }}
                                                 </div>
                                                 <div class="flex flex-col">
                                                     <span class="text-xs font-bold {{ $isMe ? 'text-emerald-400' : 'text-white' }}">
                                                         {{ $isMe ? 'You' : ($reporter ?: 'Unknown') }}
                                                     </span>
                                                     <span class="text-[10px] text-slate-500 font-medium">Patroller</span>
                                                 </div>
                                             </div>
                                             <div class="flex flex-col items-end">
                                                 <span class="text-emerald-400 text-sm font-bold">{{ $count }}</span>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            @if(!empty($stats['species']))
                            <div class="space-y-3">
                                <h4 class="text-slate-400 text-xs font-bold uppercase tracking-widest flex items-center gap-2 pl-1">
                                    <span>🦎</span> Species Stats
                                </h4>
                                <div class="bg-white/5 border border-white/5 rounded-2xl p-4 space-y-4">
                                    @php $totalSpecies = array_sum($stats['species']); @endphp
                                    @foreach($stats['species'] as $species => $count)
                                        @php
                                        ...
                                        <div class="space-y-1 group">
                                            <div class="flex justify-between text-xs">
                                                <span class="text-slate-300 font-medium group-hover:text-white transition-colors">{{ $name }}</span>
                                                <span class="text-white font-bold">{{ $count }}</span>
                                            </div>
                                            <div class="w-full h-2 bg-slate-700/50 rounded-full overflow-hidden">
                                                <div class="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full animate-progress" style="width: {{ $percent }}%"></div>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                             ...
                             <div class="space-y-3">
                                 ...
                                 <div class="bg-white/5 border border-white/5 rounded-2xl p-4">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($stats['priorities'] as $priority => $count)
                                            @php
                                            ...
                                            @endphp
                                            <div class="flex-1 min-w-[100px] flex items-center justify-between {{ $styles['bg'] }} border {{ $styles['border'] }} rounded-xl px-3 py-2 transition-transform hover:scale-105">
                                                <div class="flex items-center gap-2">
                                                    <div class="w-2 h-2 rounded-full {{ $styles['dot'] }} animate-pulse"></div>
                                                    <span class="{{ $styles['text'] }} text-xs font-bold uppercase tracking-wider">{{ $priority }}</span>
                                                </div>
