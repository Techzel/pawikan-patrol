...
@section('content')
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('patroller.reports.show', $report) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-medium transition-colors cinzel-text">
            ...
            <!-- Edit Form -->
            <form method="POST" action="{{ route('patroller.reports.update', $report) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="glass-dark rounded-xl p-6 border border-ocean-500/20">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="report_type" id="report_type" required class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text">
                                ...
                                <option value="incident" {{ old('report_type', $report->report_type) == 'incident' ? 'selected' : '' }}>Incident</option>
                                <option value="observation" {{ old('report_type', $report->report_type) == 'observation' ? 'selected' : '' }}>Observation</option>
                                <option value="maintenance" {{ old('report_type', $report->report_type) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="emergency" {{ old('report_type', $report->report_type) == 'emergency' ? 'selected' : '' }}>Emergency</option>
                            </select>
                        </div>
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $report->title) }}" required maxlength="255" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="Brief title describing the report">
                    </div>

                    <!-- Description -->
                    <div class="mt-6">
                        <label for="description" class="block text-sm font-medium text-gray-300 mb-2 cinzel-text">Description *</label>
                        <textarea name="description" id="description" rows="4" required minlength="10" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="Detailed description of the incident, observation, or maintenance issue">{{ old('description', $report->description) }}</textarea>
                    </div>
                </div>
                ...
                <div class="glass-dark rounded-xl p-6 border border-ocean-500/20">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="text" name="location" id="location" value="{{ old('location', $report->location) }}" required maxlength="255" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="e.g., Dahican Beach, Section A">
                        </div>

                        <!-- Latitude -->
                        <div>
                            <label for="latitude" class="block text-sm font-medium text-gray-300 mb-2 cinzel-text">Latitude</label>
                            <input type="number" name="latitude" id="latitude" value="{{ old('latitude', $report->latitude) }}" step="0.000001" min="-90" max="90" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="e.g., 7.123456">
                        </div>

                        <!-- Longitude -->
                        <div>
                            <label for="longitude" class="block text-sm font-medium text-gray-300 mb-2 cinzel-text">Longitude</label>
                            <input type="number" name="longitude" id="longitude" value="{{ old('longitude', $report->longitude) }}" step="0.000001" min="-180" max="180" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="e.g., 126.123456">
                        </div>
                    </div>
                ...
                <div class="glass-dark rounded-xl p-6 border border-ocean-500/20">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="turtle_count" id="turtle_count" value="{{ old('turtle_count', $report->turtle_count) }}" min="0" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="Number of turtles">
                        </div>

                        <!-- Turtle Species -->
                        <div>
                            <label for="turtle_species" class="block text-sm font-medium text-gray-300 mb-2 cinzel-text">Species</label>
                            <input type="text" name="turtle_species" id="turtle_species" value="{{ old('turtle_species', $report->turtle_species) }}" maxlength="255" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text" placeholder="e.g., Olive Ridley">
                        </div>
                ...
                <div class="glass-dark rounded-xl p-6 border border-ocean-500/20">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="datetime-local" name="incident_datetime" id="incident_datetime" value="{{ old('incident_datetime', $report->incident_datetime ? $report->incident_datetime->format('Y-m-d\TH:i') : '') }}" class="w-full bg-white/10 border border-ocean-500/30 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-ocean-500 focus:border-transparent cinzel-text">
                    </div>

                    <!-- Requires Follow-up -->
                    <div class="flex items-center">
                        <input type="checkbox" name="requires_followup" id="requires_followup" value="1" {{ old('requires_followup', $report->requires_followup) ? 'checked' : '' }} class="h-4 w-4 text-ocean-500 focus:ring-ocean-500 border-ocean-300 rounded">
